...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.log('SePay TX:', txId, '| Amount:', amount, '| Desc:', description);

    if (txId) {
      const { data: existing } = await supabase.from('processed_transactions').select('id').eq('tx_id', txId).single();
      if (existing) { console.log('Bo qua TX trung:', txId); return res.status(200).json({ success: true }); }
    }
    ...
    if (!user) {
      ...
      if (codeMatch) {
        ...
        console.log('Tim user:', withDash, 'hoac', raw);
    ...
    if (error) { console.log('Loi update:', error.message); return res.status(200).json({ success: true }); }
